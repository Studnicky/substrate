...
- Updated dependencies [92e7c65]
  - @studnicky/concurrency@11.0.1
  - @studnicky/mutex@11.0.1
...
- Updated dependencies [d05cb42]
  - @studnicky/concurrency@11.0.0
  - @studnicky/mutex@11.0.0
...
- Updated dependencies [789da06]
  ...
  - @studnicky/mutex@9.1.0

## 9.0.0

### Major Changes

- d5be000: `KeyedWorkGate<K>` stores `Coalesce<unknown>` and requires a runtime result predicate for `runSingleFlight()` and `runSerialized()`, so every caller proves its result type at the mutex and coalescing boundaries.
...
- 837480d: `KeyedWorkGate<K>` stores `Coalesce<unknown>` and requires a runtime result predicate for `runSingleFlight()` and `runSerialized()`, so every caller proves its result type at the mutex and coalescing boundaries.
