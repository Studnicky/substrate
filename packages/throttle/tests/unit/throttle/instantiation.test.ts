...
// Test fixture functions for static factory method tests
const factoryResultFn = async (): Promise<string> => {
  return 'factory-result';
};
const chainedResultFn = async (): Promise<string> => {
  return 'chained-result';
};
const multiplyAsync = async (first: number, second: number): Promise<number> => {
  return first * second;
};
