...
async function runCase(scenarioCase: ScenarioCase): Promise<void> {
  ...
  assert.equal(scenarioCase.expected.imports, true);
}

const runnerMap: Record<string, (scenarioCase: ScenarioCase) => Promise<void>> = {
  'smoke-example': runCase
};

void describe('examples smoke', () => {
  for (const scenario of scenarioGroups.cases as ScenarioCase[]) {
    void it(scenario.name, async () => {
      await runnerMap['smoke-example'](scenario);
    });
  }
});
