...
export class ValidationError extends BaseError {
   ...
   */
  public static create(argumentList: Readonly<ValidationErrorArgumentsEntity.Type>): ValidationError {
    const result = new ValidationError(argumentList);
    return result;
  }
  ...
  public get violations(): readonly Readonly<ValidationViolationDetailEntity.Type>[] | undefined {
    ...
    const violationsLength = this.#violations.length;
  ...
  protected constructor(argumentList: Readonly<ValidationErrorArgumentsEntity.Type>) {
    const metadata: Record<string, JSONSchema7Type> = { 'path': argumentList.path };
    // Violations remain a dedicated field rather than error metadata.
    super({
      'code': 'errors.validationFailed',
      'correlationId': argumentList.correlationId,
      'message': ValidationError.buildMessage(argumentList.path, argumentList.message),
      'metadata': metadata,
      'retryable': false
    });
    let violations: ValidationViolationDetailEntity.Type[] | undefined;
    if (argumentList.violations !== undefined) {
      violations = [];
      const violationsLength = argumentList.violations.length;
      for (let violationIndex = 0; violationIndex < violationsLength; violationIndex += 1) {
        const violation = argumentList.violations[violationIndex];
        if (violation === undefined) {
          continue;
        }
        const snapshot: ValidationViolationDetailEntity.Type = {
  ...
  */
  protected static buildMessage(path: string, summary: string): string {
    const result = String.raw`Validation failed at "${path}": ${summary}`;
    return result;
  }
  ...
  protected override serializeExtra(): Record<string, unknown> {
    if (this.#violations !== undefined) {
      const result: Record<string, unknown> = { 'violations': this.violations };
      return result;
    }
  ...
  protected override formatUserMessage(): string {
    if (this.#violations !== undefined && this.#violations.length > 0) {
      const lines = this.#violations.map((violation) => { const result = `  - ${violation.path}: ${violation.message}`; return result; });
