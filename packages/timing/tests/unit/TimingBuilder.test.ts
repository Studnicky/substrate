...
const invalidConfigScenarios: Array<{ description: string; build: () => unknown }> = [
  { description: 'throws ConfigurationError for invalid maxEvents', build: () => Timing.builder().maxEvents(-1).build() },
  { description: 'throws ConfigurationError for invalid precision', build: () => Timing.builder().precision({ ms: -1 }).build() },
];
for (const { description, build } of invalidConfigScenarios) {
  void it(description, () => { assert.throws(build, ConfigurationError); });
}
