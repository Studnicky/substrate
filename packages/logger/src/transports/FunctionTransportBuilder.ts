...
export class FunctionTransportBuilder {
  static create(
    create: (sink: (record: LogRecordType) => void, options: FunctionTransportOptionsType) => FunctionTransport
  ): FunctionTransportBuilder {
    return new FunctionTransportBuilder(create);
  }

  readonly #create: (sink: (record: LogRecordType) => void, options: FunctionTransportOptionsType) => FunctionTransport;
  #sink: ((record: LogRecordType) => void) | undefined;
  #options: FunctionTransportOptionsType = {};

  private constructor(create: (sink: (record: LogRecordType) => void, options: FunctionTransportOptionsType) => FunctionTransport) {
    this.#create = create;
  }

  withSink(sink: (record: LogRecordType) => void): this {
    this.#sink = sink;
    return this;
  }
