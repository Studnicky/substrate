/** patch-predicates — RFC-6902 JSON Patch, deep equality, type guards, and cycle-safe freeze. Run: npx tsx packages/json/examples/patch-predicates.ts */
...
import { Frozen, Patch, PatchError } from '../src/index.js';
import { PatchPredicatesFixture } from './fixtures/PatchPredicatesFixture.js';
...
// ---------------------------------------------------------------------------
// Predicates — deep equality and type guards
// ---------------------------------------------------------------------------

const nestedEqual = Predicates.areDeeplyEqual({ 'a': [1, 2] }, { 'a': [1, 2] });
console.log('deepEqual nested arrays:', nestedEqual);
console.log('isPlainObject({}):', Predicates.isPlainObject({}));
console.log('isRecord({a:1}):', Predicates.isRecord({ 'a': 1 }));

// ---------------------------------------------------------------------------
// Frozen — deep freeze JSON data
// ---------------------------------------------------------------------------

const tree = PatchPredicatesFixture.Tree;
...
assert.equal(Predicates.areDeeplyEqual({ 'a': 1 }, { 'a': 2 }), false, 'deepEqual detects difference');

assert.equal(Predicates.isPlainObject({}), true, 'plain object guard');
assert.equal(Predicates.isPlainObject([]), false, 'array is not plain object');
assert.equal(Predicates.isPlainObject(null), false, 'null is not plain object');
assert.equal(Predicates.isRecord([]), false, 'array is not record');
assert.equal(Predicates.isRecord({ 'a': 1 }), true, 'object is record');

assert.equal(Predicates.hasCycle({ 'a': { 'b': 1 } }), false, 'no cycle in plain object');
...
console.log('patch-predicates: all assertions passed');
