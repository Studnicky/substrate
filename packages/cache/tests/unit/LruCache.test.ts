...
it('create: staleMs entries survive staleMs without eviction', async () => {
  const cache = LruCache.create<string, number>({
    'capacity': 10,
    'staleMs': 1,
    'ttlMs': 10_000
  });
...
it('create: staleMs fires the onStale path (not onHit) once past the stale mark', async () => {
