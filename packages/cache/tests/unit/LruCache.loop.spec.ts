...
function isCacheConfig(value: unknown): value is CacheConfig {
  ...
  const config = value as { capacity?: unknown; staleMs?: unknown; ttlMs?: unknown };
  return typeof config.capacity === 'number'
    && (config.staleMs === undefined || typeof config.staleMs === 'number')
    && (config.ttlMs === undefined || typeof config.ttlMs === 'number');
}

function cacheConfig(scenarioCase: ScenarioCase): CacheConfig {
  const config = scenarioCase.input.cache;
  if (!isCacheConfig(config)) {
    throw new Error(`Invalid cache config for scenario ${scenarioCase.name}`);
  }

  return config;
}

function objectInput(value: unknown, scenarioName: string, propertyName: string): object {
  if (typeof value !== 'object' || value === null) {
    throw new Error(`Invalid ${propertyName} object for scenario ${scenarioName}`);
  }

  return value;
}

function numberArrayInput(value: unknown, scenarioName: string, propertyName: string): number[] {
  if (!Array.isArray(value)) {
    throw new Error(`Invalid ${propertyName} number array for scenario ${scenarioName}`);
  }
...
function createCache<K, V>(scenarioCase: ScenarioCase): LruCache<K, V> {
  return LruCache.create<K, V>(cacheConfig(scenarioCase));
}

function createRecordingCache(scenarioCase: ScenarioCase): RecordingCache {
  return new RecordingCache(cacheConfig(scenarioCase));
}

const runnerMap = {
  'clear-empties-cache': (scenarioCase) => {
    const cache = createCache<string, number>(scenarioCase);
    cache.set(String(scenarioCase.input.firstKey), Number(scenarioCase.input.firstValue));
    cache.set(String(scenarioCase.input.secondKey), Number(scenarioCase.input.secondValue));
    cache.clear();
    assert.strictEqual(cache.size, Number(scenarioCase.expected.size));
    assert.strictEqual(cache.get(String(scenarioCase.input.firstKey)), scenarioCase.expected.firstValue ?? undefined);
    assert.strictEqual(cache.get(String(scenarioCase.input.secondKey)), scenarioCase.expected.secondValue ?? undefined);
  },
  'delete-existing': (scenarioCase) => {
    const cache = createCache<string, number>(scenarioCase);
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.value));
    assert.strictEqual(cache.delete(String(scenarioCase.input.key)), Boolean(scenarioCase.expected.deleted));
    assert.strictEqual(cache.get(String(scenarioCase.input.key)), scenarioCase.expected.value ?? undefined);
  },
  ...
  'delete-where-matches': (scenarioCase) => {
    ...
    for (const [key, value] of scenarioCase.input.entries as Array<[string, number]>) {
      cache.set(key, value);
    }
    cache.log.length = 0;
    const removed = cache.deleteWhere((_key, value) => { return Boolean(scenarioCase.expected.matchPredicate ? value % 2 === 1 : false); });
    assert.strictEqual(removed, Number(scenarioCase.expected.removed));
    assert.strictEqual(cache.has(String(scenarioCase.expected.hasAKey)), Boolean(scenarioCase.expected.hasA));
    assert.strictEqual(cache.has(String(scenarioCase.expected.hasBKey)), Boolean(scenarioCase.expected.hasB));
    assert.strictEqual(cache.has(String(scenarioCase.expected.hasCKey)), Boolean(scenarioCase.expected.hasC));
    assert.strictEqual(cache.size, Number(scenarioCase.expected.size));
    const deleteEvents = cache.log.filter((entry) => { return entry.event === 'delete'; });
    assert.strictEqual(deleteEvents.length, Number(scenarioCase.expected.deleteCount));
  },
  ...
  'entry-ttl-overrides-global': (scenarioCase) => {
    ...
    cache.set(String(scenarioCase.input.shortKey), String(scenarioCase.input.shortValue), { ttlMs: Number(scenarioCase.input.shortTtlMs) });
    cache.set(String(scenarioCase.input.longKey), String(scenarioCase.input.longValue));
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        assert.strictEqual(cache.get(String(scenarioCase.input.shortKey)), scenarioCase.expected.shortValue ?? undefined);
        assert.strictEqual(cache.get(String(scenarioCase.input.longKey)), String(scenarioCase.expected.longValue));
        resolve();
  ...
  'evict-correct-key': (scenarioCase) => {
    const cache = createRecordingCache(scenarioCase);
    cache.set(String(scenarioCase.input.lruKey), Number(scenarioCase.input.lruValue));
    cache.log.length = 0;
    cache.set(String(scenarioCase.input.newKey), Number(scenarioCase.input.newValue));
    const evictEvents = cache.log.filter((entry) => { return entry.event === 'evict'; });
    assert.strictEqual(evictEvents.length, Number(scenarioCase.expected.evictCount));
    if (evictEvents[0]?.event === 'evict') {
      assert.strictEqual(evictEvents[0].key, String(scenarioCase.expected.evictKey));
    }
  },
  ...
  'hard-expiry-wins': (scenarioCase) => {
    ...
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        ...
        assert.deepStrictEqual(cache.log[0], scenarioCase.expected.firstLogEntry);
        assert.deepStrictEqual(cache.log[1], scenarioCase.expected.secondLogEntry);
        resolve();
      }, Number(scenarioCase.input.waitMs));
    });
  },
  'has-existing': (scenarioCase) => {
    const cache = createCache<string, number>(scenarioCase);
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.value));
    assert.strictEqual(cache.has(String(scenarioCase.input.key)), Boolean(scenarioCase.expected.has));
  },
  ...
  'lru-evicts-tail': (scenarioCase) => {
    ...
    assert.strictEqual(cache.get(String(scenarioCase.expected.evictedKey)), scenarioCase.expected.evictedValue ?? undefined);
    assert.strictEqual(cache.get(String(scenarioCase.expected.keptKey)), Number(scenarioCase.expected.keptValue));
    assert.strictEqual(cache.get(String(scenarioCase.expected.newKey)), Number(scenarioCase.expected.newValue));
  },
  'lru-promotes-accessed-entry': (scenarioCase) => {
    ...
    cache.set(String(scenarioCase.input.thirdKey), Number(scenarioCase.input.thirdValue));
    assert.strictEqual(cache.get(String(scenarioCase.expected.keptKey)), Number(scenarioCase.expected.keptValue));
    assert.strictEqual(cache.get(String(scenarioCase.expected.evictedKey)), scenarioCase.expected.evictedValue ?? undefined);
    assert.strictEqual(cache.get(String(scenarioCase.expected.newKey)), Number(scenarioCase.expected.newValue));
  },
  ...
  'object-key-identity': (scenarioCase) => {
    ...
    const keyA = objectInput(scenarioCase.input.keyA, scenarioCase.name, 'keyA');
    const keyB = objectInput(scenarioCase.input.keyB, scenarioCase.name, 'keyB');
  ...
  'on-expire-and-on-miss': (scenarioCase) => {
    const cache = createRecordingCache(scenarioCase);
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.value), { ttlMs: Number(scenarioCase.input.ttlMs) });
    cache.log.length = 0;
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        const result = cache.get(String(scenarioCase.input.key));
  ...
  'on-expire-with-has': (scenarioCase) => {
    ...
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        ...
        const expireEvents = cache.log.filter((entry) => { return entry.event === 'expire'; });
        assert.strictEqual(expireEvents.length, Number(scenarioCase.expected.expireCount));
        assert.deepStrictEqual(expireEvents[0], scenarioCase.expected.expireEntry);
        resolve();
      }, Number(scenarioCase.input.waitMs));
    });
  },
  ...
  'on-update': (scenarioCase) => {
    const cache = createRecordingCache(scenarioCase);
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.firstValue));
    cache.log.length = 0;
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.secondValue));
    assert.strictEqual(cache.log.length, Number(scenarioCase.expected.logLength));
    assert.deepStrictEqual(cache.log[0], scenarioCase.expected.logEntry);
  },
  'per-call-stale-override': (scenarioCase) => {
    const cache = createRecordingCache(scenarioCase);
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.value), { staleMs: Number(scenarioCase.input.staleMs) });
    cache.log.length = 0;
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        const result = cache.get(String(scenarioCase.input.key));
  ...
  'set-get': (scenarioCase) => {
    ...
    assert.strictEqual(cache.get(String(scenarioCase.input.key)), String(scenarioCase.expected.value));
  },
  'set-vs-update': (scenarioCase) => {
    const cache = createRecordingCache(scenarioCase);
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.firstValue));
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.secondValue));
    const sets = cache.log.filter((entry) => { return entry.event === 'set'; });
    const updates = cache.log.filter((entry) => { return entry.event === 'update'; });
    assert.strictEqual(sets.length, Number(scenarioCase.expected.setCount));
    assert.strictEqual(updates.length, Number(scenarioCase.expected.updateCount));
  },
  'size-reflects-entry-count': (scenarioCase) => {
    const cache = createCache<string, number>(scenarioCase);
    const sizes = numberArrayInput(scenarioCase.expected.sizes, scenarioCase.name, 'sizes');
    const [emptySize, firstSize, secondSize, afterDeleteSize] = sizes;
    if (emptySize === undefined || firstSize === undefined || secondSize === undefined || afterDeleteSize === undefined) {
      throw new Error(`Invalid sizes number array for scenario ${scenarioCase.name}`);
    }
  ...
  'throwing-on-expire': (scenarioCase) => {
    const input = scenarioCase.input as { cache: CacheConfig; key: string; ttlMs: number; value: number; throwMessage: string; waitMs: number };
  ...
  'ttl-before-expiry': (scenarioCase) => {
    const cache = createCache<string, number>(scenarioCase);
    cache.set(String(scenarioCase.input.key), Number(scenarioCase.input.value));
    assert.strictEqual(cache.get(String(scenarioCase.input.key)), Number(scenarioCase.expected.value));
  },
