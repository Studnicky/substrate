...
The file must exist at the given path before calling `acquire`. If the file is absent or already locked by another process, `acquire` throws `FileLockTimeoutError` after the configured timeout. The `FileLock` instance exposes `read` and `write` for operating on the locked file, and `release` (or `Symbol.dispose`) to return it to its original path.
...
const lock = await FileLock.acquire('/data/config.json');
...
{
  await using lock = await FileLock.acquire('/data/config.json');
...
const lock = await FileLock.acquire('/data/config.json', {
  pollMs: 50,      // check every 50 ms (default: 100)
  timeoutMs: 5000, // give up after 5 s (default: 30 000)
});
...
try {
  const lock = await FileLock.acquire('/data/config.json', { timeoutMs: 2000 });
  try {
    lock.write('updated');
  } finally {
    lock.release();
