...
- d5be000: `EntryEntity.Type` is the schema-derived filesystem entry data. `VirtualFileSystemOptionsInterface`, `FileSystemInterface`, and `StatResultInterface` define runtime and readonly contracts. `@studnicky/virtual-fs` is the sole public code entrypoint.
...
- 837480d: `EntryEntity.Type` is the schema-derived filesystem entry data. `VirtualFileSystemOptionsInterface`, `FileSystemInterface`, and `StatResultInterface` define runtime and readonly contracts. `@studnicky/virtual-fs` is the sole public code entrypoint.
