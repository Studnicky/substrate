...
void it('DeepMergeType module resolves and type is usable in assignments', () => {
...
void it('DeepMergeType overlay wins on conflicting keys', () => {
  type Merged = DeepMergeType<{ x: number }, { x: string }>;
  const merged: Merged = { x: 'overlay' };
  assert.equal(typeof merged.x, 'string');
});

void it('DeepMergeType base keys not in overlay are preserved', () => {
