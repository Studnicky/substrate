...
export function areObjectsEqual(
  ...
  for (const key of keys1) {
    ...
    if (!compareDeep(value[key], filterValue[key], condition)) {
      return false;
    }
  }
...
function compareDeep(value: unknown, filterValue: unknown, condition: FilterCondition = {}) : boolean {
  ...
  if (typeof value === 'object' && typeof filterValue === 'object') {
    return areObjectsEqual(value as Record<string, unknown>, filterValue as Record<string, unknown>, condition);
  }
