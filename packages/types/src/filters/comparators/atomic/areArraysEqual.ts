...
export function areArraysEqual(value: unknown[], filterValue: unknown[], condition: FilterCondition = {}) : boolean {
  ...
  for (let i = 0; i < valueLength; i++) {
    ...
    if (!compareDeep(value[i], filterValue[i], condition)) {
      return false;
    }
  }
 ...
 */
function compareDeep(value: unknown, filterValue: unknown, condition: FilterCondition = {}) : boolean {
  ...
  if (Array.isArray(value) && Array.isArray(filterValue)) {
    return areArraysEqual(value, filterValue, condition);
  }
  ...
  if (typeof value === 'object' && typeof filterValue === 'object') {
    ...
    for (const key of keys1) {
      ...
      if (!compareDeep(valueObj[key], filterObj[key], condition)) {
        return false;
      }
    }
