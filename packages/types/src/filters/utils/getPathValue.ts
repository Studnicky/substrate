 ...
 */
function isSafeProperty(propertyName: string): boolean {
 ...
 */
function readField(current: unknown, key: string): unknown {
 ...
 */
function processArrayIndexing(part: string, current: unknown, path: string, parts: string[], index: number): { 'isWildcard': boolean;
  'value': unknown } {
  const [
    fieldName,
    indexPart
  ...
  const arrayValue = readField(current, fieldName);
 ...
 */
export function getPathValue(obj: FilterValue, path: string, maxDepth?: number): FilterValue | ArrayWildcardValue {
  if (!path) {
    return obj;
  }
  ...
  for (let i = 0; i < partsLength; i++) {
    ...
    if (part.includes('[') && part.includes(']')) {
      const result = processArrayIndexing(part, current, path, parts, i);
