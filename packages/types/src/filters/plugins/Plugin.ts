...
export abstract class Plugin implements BasePluginInterface {
   ...
   */
  protected getConfig<T>(key: string, options: { 'context'?: PluginContextInterface; 'defaultValue'?: T } = {}): T | undefined {
    const result = (options.context?.options?.[key] as T) ?? (this.defaultConfig[key] as T) ?? options.defaultValue;
