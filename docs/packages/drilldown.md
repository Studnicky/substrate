...
Use the rule entity type when your application constructs a reusable rule. The named nested types select one branch when an integration needs to work with a specific matcher.

```ts
import type { DrilldownRulesEntity } from "@studnicky/drilldown/entities";

const rule: DrilldownRulesEntity.GroupRuleEntity.Type = {
  property: "category",
  values: [{ match: "alpha", type: "string" }]
};
```
...
| `GroupValueUnionType` | Union of all group-value shapes accepted by `DrilldownRulesEntity.GroupRuleEntity.Type`. | `@studnicky/drilldown/types` |
