...
assert_contains "sync workflow creates the disposable branch from main" 'git switch --create "$branch" main' "$workflow"
assert_contains "sync workflow adds the synchronization changeset" '.changeset/sync-main-to-develop.md' "$workflow"
assert_contains "sync workflow pushes only the disposable branch" 'git push origin "HEAD:refs/heads/$branch"' "$workflow"
