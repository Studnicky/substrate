...
assert_contains "sync workflow keeps the changeset format inline" "printf -- '---\n---\n\nSyncs the release commits on " "$workflow"
assert_contains "sync workflow closes the inline changeset format" "No package changes.\n' > .changeset/sync-main-to-develop.md" "$workflow"
...
assert_contains "sync workflow pins the automatic merge to the pushed head" 'gh pr merge "$PR_NUMBER" --repo "$GITHUB_REPOSITORY" --merge --auto --delete-branch --match-head-commit "$SYNC_SHA"' "$workflow"
assert_contains "sync workflow waits for required checks before a direct merge" 'gh pr checks "$PR_NUMBER" --repo "$GITHUB_REPOSITORY" --required --watch' "$workflow"
